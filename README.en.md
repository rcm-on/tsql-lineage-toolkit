...
In addition, **441 tests (xUnit)** cover the parser and **all of them run as a gate in CI**. Three are in the `LiveSql` category and check against a live SQL Server: CI spins up a container with WideWorldImporters and AdventureWorks2019 restored ([`eng/restore-sample-databases.sh`](eng/restore-sample-databases.sh)).
...
- **And that 99.7 % is measured over 95.5 % of the corpus, not all of it.** The oracle is `sys.dm_sql_referenced_entities`, and a module with a single broken reference — a dropped object, a cross-database call, a linked server — loses **every** row there, including the ones that did resolve. In DNN, 706 of 739 programmable modules contribute anything to the oracle. The more broken a database is, the better that percentage looks, so the engine now publishes the oracle's own coverage next to the figure instead of quoting recall on its own: `recall` warns you when it is measuring over a fraction.
