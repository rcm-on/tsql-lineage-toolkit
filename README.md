...
- **El lineage de columna no es completo.** 97,0 % de cobertura medida sobre corpus grande, y 248 referencias que no vemos. Están [documentadas](eval/column-recall/).
...
> la cobertura real es del **97,0 %**, y quedan **248 referencias que el motor no
...
- **El lineage de columna es del 97,0 %, no del 100 %.** Medido sobre 7.786 referencias de un corpus de producción (DNN Platform), y 248 referencias que no se ven. La ausencia de una arista es "no detectada", no "probado que no existe". Ver [`eval/column-recall/`](eval/column-recall/).
