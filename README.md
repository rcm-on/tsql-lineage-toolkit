...
- **El lineage de columna no es completo.** 96,0 % de cobertura medida sobre corpus grande, y 318 referencias que no vemos. Están [documentadas](eval/column-recall/).
...
> la cobertura real es del **96,0 %**, y quedan **318 referencias que el motor no
...
| Expandida de un `SELECT *` | 1.663 | **98,1 %** |
...
- **El lineage de columna es del 96,0 %, no del 100 %.** Medido sobre 7.786 referencias de un corpus de producción (DNN Platform), y 318 referencias que no se ven. La ausencia de una arista es "no detectada", no "probado que no existe". Ver [`eval/column-recall/`](eval/column-recall/).
